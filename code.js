anychart.onDocumentReady(function () {
    ...
    function createChart(dosing_schedule_data) {
        ...
        body_concentration =
            body_concentration * dailyDecayFactor(halfLifePeriod);
        ...
        body_concentration_data.push([
            dateString,
            body_concentration_nmol.toFixed(2),
        ]);
        });
        ...
        var seriesBodyConcentration = chart.line(
        anychart.data.set(body_concentration_data).mapAs({ x: 0, value: 1 })
        );
        ...
        return [
            entry[0],
            correspondingBodyConcentration
            ? correspondingBodyConcentration[1]
            : entry[1],
        ];
        });

        var seriesDosingSchedule = chart.marker(
        anychart.data.set(adjusted_dosing_schedule_data).mapAs({ x: 0, value: 1 })
        );
        seriesDosingSchedule.name("Dosing schedule");
        seriesDosingSchedule.tooltip().enabled(false);
        seriesDosingSchedule
        .labels()
        .enabled(true)
        .anchor("left-center")
        .padding(10)
        ...
        var seriesMovingAverage = chart.line(
        anychart.data.set(moving_average_data).mapAs({ x: 0, value: 1 })
        );
        seriesMovingAverage.name("14-day Moving Average");
        seriesMovingAverage.stroke({
        dash: "5 2",
        thickness: 2,
        color: "#FF5733",
        });
